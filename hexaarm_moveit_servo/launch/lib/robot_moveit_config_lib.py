...
def get_xarm_robot_description_parameters(
    xacro_urdf_file=PathJoinSubstitution([FindPackageShare('hexaarm_description'), 'urdf', 'hexaarm.urdf.xacro']),
    xacro_srdf_file=PathJoinSubstitution([FindPackageShare('hexaarm_moveit'), 'srdf', 'hexaarm.srdf']),
    urdf_arguments={},
    srdf_arguments={},
    arguments={}):
    urdf_arguments['ros2_control_plugin'] = urdf_arguments.get('ros2_control_plugin', 'uf_robot_hardware/UFRobotSystemHardware')
    moveit_config_package_name = 'hexaarm_moveit'
    hexaarm_type = arguments.get('hexaarm_type', None)
    
    # xarm_description/launch/lib/robot_description_lib.py
    mod = load_python_launch_file_as_module(os.path.join(get_package_share_directory('hexaarm_moveit_servo'), 'launch', 'lib', 'robot_description_lib.py'))
    ...
    return {
        ...
        'robot_description_planning': load_yaml(moveit_config_package_name, 'config', hexaarm_type, 'joint_limits.yaml'),
        'robot_description_kinematics': load_yaml(moveit_config_package_name, 'config', hexaarm_type, 'kinematics.yaml')
    }
